...
class ComparedModel:
    ...
    def compare(self, c_model):
        self.results['comparing'] = f'{self.ep_name} at {self.released} vs {c_model.ep_name} at {c_model.released}'
        self.results['paths'] = []
        ...
        for n_id in set(self.G_conns.keys()) - set(c_model.G_conns.keys()):
            result = {'id': n_id, 'is_isomorphic': False if is_isomorphic else False, 'status':f'not found in {self.ep_name}'}
        ...
        for n_id in set(c_model.G_conns.keys()) - set(self.G_conns.keys()):
            result = {'id': n_id, 'is_isomorphic': False if is_isomorphic else False, 'status':f'not found in {c_model.ep_name}'}
...
def print_tabular(results, kb1:ComparedModel, kb2:ComparedModel):
    print(f"{results['model']}, {kb1.ep_name} at {kb1.released}, {kb2.ep_name} at {kb2.released}")
    for path in results['paths']:
        for edge in path['edges']:
            for ep, e in edge.items():
                ...
                ne_str = '-'.join(ne)
                print(f"{path['id']}, {ne_str if ep==kb1.ep_name else ''}, {ne_str if ep==kb2.ep_name else ''}")
